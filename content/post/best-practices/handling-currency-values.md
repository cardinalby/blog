...
9. For some currencies there are no [physical denomination](https://en.wikipedia.org/wiki/Cash_rounding) for 
the minor unit.
10. Storing prices of small units of goods (probably as a result of conversion from another currency) can require 
using **more decimals** than is defined for a currency.

### Storage requirements:
1. Obvious one: store currency amounts **together with** a link to the currency **definition** 
(foreign key in databases, special class in programming languages) to interpret and operate with it correctly.
3. Storing a **definition** (spec) for a currency you should include:
   - **_Minimum accountable unit_** instead of or together with **precision** (see _fact 5_).
   - **_Lowest physical denomination_** of the currency if you deal with cash operations (see _fact 9_).
4. Ensure **precision** for currency amounts equals to max precision of all supported currencies.
5. Consider adding **extra precision** for operational needs: accumulators and intermediate calculations or for storing 
prices of small units of goods. For example, you may want to accumulate a _10 %_ fee from 1 cent operations, 
sum them up until they reach _minimal accountable unit_ (cent) and withdraw from a client.

## Data types
There are different data types that can technically store money values. Let's see how the listed requirements can be 
fulfilled by utilizing different data types.

## 1️⃣ Integer number of minor units
One of the popular ([Stripe](https://stripe.com/docs/currencies#zero-decimal)) 
[approaches](https://en.wikipedia.org/wiki/Fixed-point_arithmetic) is storing an integer 
number of minor units. Simply put, you store **_5 $_** as **_500 cents_**. This way you can do accurate 
calculations and comparisons internally and then display the result formatting the number in a proper way as an amount 
of dollars. 

Taking into consideration the requirement about **extra precision** (let's say 3 extra decimals) you will represent 
**_5 $_** as **_500 000_** of **"micro units"**, where **_1_** _micro unit_ equals **_1 000 cents_**. 

### Issues and limitations:
- It's preferable to consider on the **precision beforehand**.
- It's **complicates** the business logic of the application and introduces **error-prone** values **conversions** between units, 
micro-units and normal currency amounts that are presented to a user or external systems.
- Due to the _fact 7_ (_minor unit_ of a currency can change) or because of the need to increase _extra precision_ 
you may need to **rescale** all values in the future.
- External systems you interact with can **misinterpret** the **magnitude** of an integer-represented 
  ...
  - Your own services that can't be immediately updated with the new definitions of the currencies 
    (limitation of the deployment process, caches).
...
It's a not-native data type that handles integers of arbitrary (or big enough) size and provide math operations on it.
It's **the most suitable** way of storing 
_minor units_ or _micro units_. **Don't confuse** it with SQL bigint that in fact is  
_Int64_. 
...
For correct **(de)serialization** you will probably need to use **strings** for compatibility between different 
implementations.

Usually, more memory is required by _BigInt_ type comparing with native integers and computations take longer 
because CPUs don't have hardware support for these data type.
...
Some choose signed or unsigned **_Int64_** (also referenced as _BigInt_ in SQL which can lead to confusion) for 
storing their cents or smaller subunits of currency. Even though at the first glance you can decide that it
has enough capacity for your use-case, I want to highlight the following **issues** with it:

- It may not be sufficient for storing minimal units of **cryptocurrencies** (see _fact 6_) or values 
with **extended precision** (see _requirement 5_) 
- Some languages require casting to _Float64_ for math operations ([Go](https://pkg.go.dev/math)).
...
- **Not all** programming **languages support** _Int64_ values:
  - PHP running on x32 architectures can't handle _Int64_ values.
  - Some languages (Java, PHP) dont support any unsigned integers.
  - JavaScript uses signed _Float64_ as internal representation for _number_ data type. It means that even if you can 
    serialize _Int64_ number to JSON in your backend application, by default a JavaScript application will
    overflow its number type trying to deserialize JSON containing this value.

The problem with support of _Int64_ in external systems can be **mitigated** by serializing _Int64_ **to string** and 
using _BigInt_ types to handle these values, but it reduces the benefits from using hardware-supported _Int64_ values.
...
This approach implies using special _Decimal_ numeric type that allows to store **fractional 
decimal numbers** accurately with the specified precision (maximum precision differs in different databases).
...
- _**Decimal128**_ with limited number of significant digits (
[MongoDB](https://www.mongodb.com/developer/products/mongodb/bson-data-types-decimal128/), 
[C#](https://learn.microsoft.com/en-us/dotnet/api/system.decimal?view=net-7.0)).
- So-called _**BigDecimal**_ type of arbitrary (or big enough) size (SQL Decimal, BigDecimal in Java) 
with different internal representations
 
> As in case of BigInt, the **binary representation** can differ: 
> - 2 decimal digits in each byte (base 100, like it's done in databases).
> - _BigInt_ with exponent in the manner similar to base 2 floating point values.
> - Single decimal digits in array or string.
...
- **No major overhead** if values are already stored as _Decimals_ in the database (you are going to do it anyway even if
  you store _BigInts_). Also, to format _BigInt_ values you will do computations similar to ones inside _Decimal_ type.
- More **intuitive** representation of the values **simplifies** business-logic and formatting numbers to 
  human-readable format.
- Changing of minor units can be done by altering the precision of _decimal_ column in the database; You 
  **don't have to rescale** the values.
- Value **serialized** to string naturally **includes** the information about **precision**. 
  If you change the precision, it **cannot be misinterpreted** by external systems as in case of _BigInts_.

As in case of _BigInt_ values, all **serialization** goes through **decimal strings** for compatibility between 
different libraries. Of course, serializing in form of mantissa + exponent can be more efficient for 
performance-sensitive applications.

However, you still need to keep track of minimal accountable units. Limiting the precision of _Decimal_ in the 
database remember about **precision reserve** for intermediate operations and accumulators.
